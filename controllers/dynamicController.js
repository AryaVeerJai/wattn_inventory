...
const createDynamicDocument = async (req, res) => {
  console.log(req.body)
  try {
    // console.log(req.body)
    const newDynamicDoc = new DynamicModel(req.body);
...
const getDynamicDocumentById = async (req, res) => {
  try {
    const doc = await DynamicModel.findById(req.params.id);
    if (!doc) {
...
const getAllDynamicDocuments = async (req, res) => {
    try {
      const docs = await DynamicModel.find(); // Find all dynamic documents
